...
- **引擎函数**：从 `graphics-icon/imagemin` import `optimizeImages` 在脚本里压缩任意文件列表。
- **CLI**：`g-min`，常用于 **pre-commit 就地压缩源图**或全量扫描。
...
```bash
g-min <files...>                       # 指定文件（如 pre-commit 暂存的图片）
g-min --all [目录...]                   # 全量扫描（缺省扫整个仓库根）
g-min --all --config ./imagemin.config.ts   # 用项目配置覆盖默认值（default-export）
```
...
| `runCli(argv)` | `(argv?: string[]) => Promise<void>` | CLI 入口（被 `g-min` 复用）。 |
