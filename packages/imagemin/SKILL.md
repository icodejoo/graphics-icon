...
### CLI（本包 bin: `codejoo-imagemin`；经伞包发布为 `graphics-icon-imagemin`）
CLI 逻辑在 `src/bin.ts`,导出 **`runCli(argv)`**（经 index 再导出），伞包 bin `graphics-icon-imagemin` 即薄包装它——
其它三引擎(colorfont/bitmap/svg)的 CLI 也照此模式由伞包统一暴露,四者能力对齐。
- 指定文件（如 pre-commit 暂存图）：`graphics-icon-imagemin a.png b.svg`
- 全量扫描：`graphics-icon-imagemin --all [目录...]`（缺省扫描 `process.cwd()`）；`--scan` 同义。
- 自定义配置：`graphics-icon-imagemin --all --config ./imagemin.config.ts`（动态 import 后浅合并到 `defaultOptions` 之上，用户优先）。
- 只「压缩 + 更新缓存」，不碰 git。
