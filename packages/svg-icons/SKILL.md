---
name: svg-icons
description: 在 D:\workspaces\colorfont 上维护 svg-icons 时参考。涵盖该包的目的、功能、双形态（引擎 generateSvgSprites + CLI runCli + 内部 Vite 插件工厂，统一经伞包 vite-plugin-graphics-icon 出口）、用法（含 vite.config 示例与消费 import { iconsHref, iconsName, type IconName }）、关键考量（共享 utils、buildStamp 指纹、共享缓存目录、id 作用域化、颜色改写、normalize 与 colorfont 同步、按需加载）与易踩的点。
---
...
本包（私有内部包）建立在第三方 `vite-plugin-icons-spritesheet` 之上，补齐它缺失/有坑的能力，是 graphics-icon monorepo 四引擎之一，与 colorfont 套件共享基础设施。

**双形态**（与 imagemin 对齐）：引擎 `generateSvgSprites(options)`（Vite 之外一次性生成）+ CLI `runCli`（经伞包 bin `graphics-icon-svg --config <file>`）+ 内部 Vite 插件工厂 `svgIcons(opts): Plugin[]`（经 `graphicsIcon({ svgIcons })` 集成，不单独对外导出）。
...
export default defineConfig({
  plugins: [
    graphicsIcon({
      // 省略 cacheFile → 落共享缓存目录 .cache.graphics/svg-icons.json
      svgIcons: { sprites: [
        {
...
单独使用(Vite 之外)：`import { generateSvgSprites } from "svg-icons"`（或经伞包 `import { svgIcons } from "vite-plugin-graphics-icon"` → `svgIcons.generate(opts)`）。
...
- 共享 utils：`buildStamp`（`@codejoo/utils/fingerprint`，缓存指纹约定，与 bitmap-icons 同源）/ `loadCache` / `saveCache` / `pruneCache` / `relTo` / `writeTextIfChanged` / `normalizeSvg` 全部来自 `@codejoo/utils`（子路径引入，无扩展名），与 bitmap / imagemin 同源，勿在本包重复实现。`computeStamp` 已改用 `buildStamp`（不再直接拼 `sha256`）。
- 共享缓存目录：缓存文件经 `resolveCacheFile('svg-icons', options.cacheFile)` 解析，默认落 `.cache.graphics/svg-icons.json`（随仓库提交 → 团队共享）。`pruneCache(cacheFile, configs.map(c => c.output.svg), '[svg-icons]')` 在首个 `buildStart` 剪枝失效条目。
...
- 按需加载：`vite-plugin-icons-spritesheet` 通过动态 `import()` 延迟到 `buildStart`，仅 `import { svgIcons }` 不会即时拉起底层插件；normalize 的重依赖（svgo/svgpath）也在 `@codejoo/utils/scale-svg` 内部惰性加载。
