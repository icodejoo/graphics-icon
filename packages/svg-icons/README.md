...
- **单独使用**：从 `graphics-icon/svg` import 引擎函数，或用 CLI（`g-svg`）——见下文。
...
```bash
g-svg --config ./svg.config.ts   # 配置文件 default-export 一个含 items[] 的 SvgIconsOptions
```
...
| API | 类型 | 作用 |
| --- | --- | --- |
| `generateSvgSprites(options)` | `(o: SvgIconsOptions) => Promise<void>` | 按 `items[]` 一次性生成所有 SVG 雪碧图 + 类型化脚本，维护每实例缓存。 |
| `svgIcons(options)` | `(o: SvgIconsOptions) => Plugin[]` | Vite 插件工厂（供伞插件内部使用；推荐经 `graphicsIcon` 使用）。 |
| `runCli(argv)` | `(argv: string[]) => Promise<void>` | CLI 入口（被 `g-svg` 复用）。 |
