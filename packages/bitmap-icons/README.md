...
- **单独使用**：从 `graphics-icon/bitmap` import 引擎函数，或用 CLI（`g-bitmap`）——见下文。
...
```bash
g-bitmap --config ./bitmap.config.ts   # 配置文件 default-export 一个含 items[] 的 BitmapIconsOptions
```
...
| API | 类型 | 作用 |
| --- | --- | --- |
| `generateBitmapSheets(options)` | `(o: BitmapIconsOptions) => Promise<void>` | 按 `items[]` 顺序生成所有图集 + 边车，维护每实例缓存（默认 `throwable` 出错即抛出）。 |
| `bitmapIcons(options)` | `(o: BitmapIconsOptions) => Plugin` | Vite 插件工厂（供伞插件内部使用；推荐经 `graphicsIcon` 使用）。 |
| `runCli(argv)` | `(argv: string[]) => Promise<void>` | CLI 入口（被 `g-bitmap` 复用）。 |
