...
类型化虚拟模块、稳定码位。纯 JS、无 node-gyp、安装期零原生编译。面向 npm 社区开源。
现为 `@codejoo/graphics-to-font` 套件成员,与 bitmap-sprite / svg-sprite / imagemin / utils 平级。

## 结构(引擎-only 私有包 `@codejoo/colorfont`,Vite 插件已拆到 `packages/vite-plugin`)
- `src/`(引擎):`index.ts` 入口(`exports["."]` = 源码,非 dist),`pipeline/`、`glyf/`、`colrv1/`、
  `flavors/`、`emit/`、`encode/`、`outline/`、`codepoints/`、`cache/`、`workers/`、`util/` 等。
- Vite 插件薄封装已**移出**到 `packages/vite-plugin`(`vite-plugin-graphics-icon`,唯一发布物),从 `@codejoo/colorfont` 导入引擎。
- `src/cli/`:build/watch/check。`run(argv)` 在 `cli/cli.ts`,经 index 以 **`runCli`** 再导出(供伞包 bin
  `graphics-icon-colorfont` 复用)。引擎双形态:`build`/`buildAndWrite`(可编程)+ `runCli`(CLI)。
- `colrv1-writer/`(Rust→wasm):COLRv1 写表。`woff2`(Rust→wasm):可调质量 woff2 编码。
- `scripts/`、`test/cli.test.ts`、`fixtures/`、`README.md`。无 tsup(伞包 tsup 内联本包源码)。

引擎不依赖 vite;插件层在伞包 `packages/vite-plugin`(`colorfont-plugin.ts` 定义插件选项类型 **`ColorfontOptions`**,
extends 引擎选项;引擎 `types.ts` 的同名类型在伞包 import 处别名 `ColorfontEngineOptions`)。
公共第三方依赖经根 `pnpm-workspace.yaml` 的 `catalog:` 统一版本。**本包现依赖 `@codejoo/utils`**(归一化/幂等写入)。

## 构建管线(每次 build,要点)
loadIcons 读 SVG → assignCodepoints(码位锁 codepoints.json,PUA 0xE000,墓碑不回收)→ 构建缓存检查
...
→ 各档 toWoff2/toWoff → `buildAndWrite` 用 `@codejoo/utils/fs-write` 幂等写入 → emitCss(双 @font-face + tech 链)+ emitDts。
...
- 伞包 tsup 内联本包源码但**不**内联第三方依赖 → 这些依赖(svg2ttf/svgo/svgpath/cubic2quad/opentype.js/ttf2woff)
  必须列在本包 `dependencies`,vite 为 peer。**`fflate` 已移除**(未使用)。
...
伞层细节见 `packages/vite-plugin/SKILL.md`(name: graphics-icon)。
