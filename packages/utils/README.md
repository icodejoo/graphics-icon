...
`colorfont`、`bitmap-icons`、`svg-icons`、`imagemin` 各引擎共用的公共函数都集中在这里，做到「一处实现、多处复用」：相似逻辑不再各写一份。本包不对外发布（`private`），仅供 monorepo 内部以 `workspace:*` 引用（其类型会在伞包 `vite-plugin-graphics-icon` 打包时内联，对最终消费方透明）。
...
| `./cache` | `resolveCacheFile` `loadCache` `saveCache` `pruneCache` `CACHE_DIR` | 共享缓存目录 + 「键→字符串」JSON 原语 + 剪枝。 |
| `./scale-svg` | `scaleSvgToWidth` `normalizeSvg` | SVG 放大（scale-that-svg）与 colorfont 对齐的归一化。 |

## 两个关键设计

**共享缓存目录**：所有缓存默认落在仓库根 `.cache.graphics/`，每个子项目缓存文件默认名 = 子项目名（`bitmap-sprite.json` / `imagemin.json` / `svg-sprite.json`）。`resolveCacheFile(name, custom?)` 支持：省略→默认名；裸文件名→仍落共享目录；含路径分隔符→完全自定义位置。
...
All functions shared by the engines (`colorfont`, `bitmap-icons`, `svg-icons`, `imagemin`) live here — implemented once, reused everywhere, so similar logic is never duplicated. This package is **private** and consumed only inside the monorepo via `workspace:*` (its types are inlined when the umbrella `vite-plugin-graphics-icon` bundles, transparent to end consumers).
...
| `./cache` | `resolveCacheFile`, `loadCache`, `saveCache`, `pruneCache`, `CACHE_DIR` | Shared cache folder + "key→string" JSON primitives + pruning. |
...
**Shared cache folder** — every cache defaults into `.cache.graphics/` at the repo root; each sub-project's file defaults to its own name. `resolveCacheFile(name, custom?)` supports: omitted → default name; bare filename → still in the shared folder; path with separators → fully custom location.
