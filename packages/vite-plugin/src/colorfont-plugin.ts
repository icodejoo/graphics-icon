import { build as coreBuild } from '@codejoo/colorfont'

import { buildGalleryHtml } from './gallery.ts'

import type { BuildResult, FontAsset, ColorfontOptions as ColorfontEngineOptions } from '@codejoo/colorfont'

/**
 * colorfont Vite 插件选项 = 引擎选项 + 插件层选项。命名与 BitmapIconsOptions / SvgIconsOptions 风格一致。
 * colorfont Vite-plugin options = engine options + plugin-only fields. Named consistently with the others.
 */
export interface ColorfontOptions extends ColorfontEngineOptions {
  /** 虚拟 CSS 模块 id,默认 'virtual:colorfont.css'。 / Virtual CSS module id. */
  cssModuleId?: string
  /** 虚拟 API 模块 id,默认 'virtual:colorfont'。 / Virtual API module id. */
  apiModuleId?: string
  /** dev 期是否监听 .svg 源变更并热重载,默认 true。 / Watch sources in dev. */
  watch?: boolean
  /** dev 极速档:woff2 用 q9,默认 true。 / Dev fast woff2. */
  devFast?: boolean
  /** build 期额外产出独立 CSS + 类型化 API + 自包含画廊,默认 false。 / Emit standalone demo gallery. */
  emitDemo?: boolean
}
...
export default function colorfont(options: ColorfontOptions): VitePluginLike {
