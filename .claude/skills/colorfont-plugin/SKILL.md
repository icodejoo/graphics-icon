...
description: >-
  vite-plugin-graphics-icon monorepo(伞插件 colorfont + bitmap-icons + svg-icons + imagemin)的整体架构、
  colorfont 引擎的关键选型抉择与实测结果。在 D:\workspaces\colorfont 上工作(改引擎/写表/wasm/打包/性能/伞层)时参考本 skill。
---

# vite-plugin-graphics-icon monorepo —— 总览 / colorfont 引擎选型 / 实测

`D:\workspaces\colorfont` 是独立 pnpm monorepo(不在 let188 仓库)。对外**唯一发布物**是伞插件
`vite-plugin-graphics-icon`,把四个引擎合成**一个** Vite 插件;其余包均 `private`,经 tsup `noExternal` 内联进伞包 dist。

## 包结构(6 包)
- `vite-plugin-graphics-icon`(packages/vite-plugin)—— **唯一发布**的伞插件 + 4 个 CLI bin。详见 `graphics-icon` skill。
- `@codejoo/colorfont` —— 彩色 webfont 引擎(纯 JS,本 skill 的深挖对象)+ CLI(`run`)。
- `bitmap-icons` —— 位图雪碧图引擎(`generateBitmapSheets`)+ 插件工厂。
- `svg-icons` —— SVG 雪碧图引擎(`generateSvgSprites`)+ 插件工厂。
- `@codejoo/imagemin` —— 图片压缩引擎(`optimizeImages`)+ CLI。
- `@codejoo/utils` —— 内部公共子模块(hash/fingerprint/glob/path-rel/fs-write/cache/scale-svg)。
- colorfont 内含两个 Rust→wasm crate:`colrv1-writer`(COLRv1 写表)、`woff2`(可调质量 woff2 编码,ttf2woff2)。

**统一形态**:四引擎都「双形态」——引擎函数 + CLI 可在 Vite 外单独用,也经 `graphicsIcon({...})` 集成。`graphicsIcon`
返回**单个**插件(钩子多路复用),按传入子键启停。各包另有自己的 SKILL.md。
...
6. `buildAndWrite` 用 `@codejoo/utils/fs-write` 的幂等写入落盘(内容未变不写,防 mtime/git/HMR 抖动)。
   `emitCss`(双 @font-face + tech 链)+ `emitDts`(IconName 联合 + icons/baseName/colorIcons)。
...
- **dev 冷启动不阻塞**:plugin `buildStart` 不 await(后台生成),`load`/字体中间件/HMR 按需 await。
- **稳定码位**:PUA 从 0xE000,墓碑策略,`codepoints.json` 提交。
...
`{"type":"commonjs"}`**。伞包 `build` = tsup + `scripts/copy-wasm.mjs`(把两个 pkg 拷进 dist/woff2、dist/colrv1)。
...
- colorfont 插件选项类型叫 `ColorfontOptions`(在伞包 `colorfont-plugin.ts`),引擎同名类型在 import 处别名
  `ColorfontEngineOptions`;旧名 `VitePluginColorfontOptions` 已废弃。
- 伞包 `dependencies` 须列全各引擎的第三方运行时依赖(svg2ttf/svgo/svgpath/cubic2quad/opentype.js/ttf2woff/
